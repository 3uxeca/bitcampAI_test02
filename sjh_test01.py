...
def build_network(keep_prob=0.5, optimizer='rmsprop'):
    ...
    model.compile(optimizer=optimizer, loss='categorical_crossentropy',
                  metrics=['accuracy'])
    model.fit_generator(data_generator.flow(X_train, y_train, batch_size=4), # 훈련을 시키되, 발전기도 같이 실행해라!
                        steps_per_epoch= 30, # 몇 배로 증폭시킬건지 steps*batch_size 자기 숫자만큼의 새로운 이미지 데이터 생성
                        epochs=200,
                        validation_data=(X_test, y_test),
                        verbose=1) 
    return model
